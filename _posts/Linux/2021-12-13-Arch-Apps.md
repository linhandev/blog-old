...
LibreWolf是基于firefox开发的一个关注保护用户隐私的浏览器，用起来体验和firefox区别不大。默认关闭浏览器后就会清除cookie所以网站会要求重新登陆，如果用了密码管理工具并不影响使用体验。大多数firefox的功能都是在的可以根据自己的需要开启保存浏览历史或者关闭浏览器不清除cookie。
```shell
yay -S librewolf-bin
```
